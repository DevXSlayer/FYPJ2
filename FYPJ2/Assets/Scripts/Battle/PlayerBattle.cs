...
public class PlayerBattle : MonoBehaviour {
    ...
    void Update () {
        ...
        else if (!charSelected && !TeamManager.Instance.CheckCharSelection())
        {
            ...
            Time.timeScale = 1.0f;
            Debug.Log("CALLING : " + gameObject.name);
        }
	}

    public void OnCharacterSelect() {
        if (!TeamManager.Instance.CheckCharSelection() |charSelected)
            SetCharSelect(!charSelected);
            
    }

    public void SetCharSelect(bool selection) { charSelected = selection; }
    public bool GetCharSelect() { return this.charSelected; }
